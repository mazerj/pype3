...
if __name__ == '__main__':
	...
	xd, yd, zd = [], [], []
	xa = np.arange(-5.0, 5.0, 1.0)
	ya = np.arange(-5.0, 5.0, 1.0)
	m  = np.zeros((len(xa), len(ya)))
	mx  = np.zeros((len(xa), len(ya)))
	my  = np.zeros((len(xa), len(ya)))
	...
	nx = np.arange(-5.0, 5.0, 0.5)
	ny = np.arange(-5.0, 5.0, 0.5)
	nz, nx, ny = griddata2(np.array(xd), np.array(yd), np.array(zd), nx, ny)

	plt.subplot(2,1,2)
	plt.contour(nx, ny, nz)
	plt.hold(True)
	plt.plot(nx, ny, 'k.')
	plt.hold(False)
